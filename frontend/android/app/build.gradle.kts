...
// If the file is absent (e.g. a fresh CI clone without the keystore), fall back
// to debug signing so the build still succeeds.
...
android {
    namespace = "com.example.secretchat"
    ...
    buildTypes {
        release {
            // Use the real release keystore when available; otherwise debug-sign
            // so `flutter build`/`flutter run --release` still works locally/CI.
